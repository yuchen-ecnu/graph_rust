...
impl<'a> HdfsFs<'a> {
  ...
  pub fn chmod(&self, path: &str, mode: i16) -> bool {
    (unsafe {
      let mut s_path = String::from(path);
      hdfsChmod(self.raw, str_to_chars(&mut s_path), mode as c_short)}) == 0
  }
  ...
  pub fn chown(&self, path: &str, owner: &str, group: &str) -> bool {
    (unsafe {
      ...
      hdfsChown(self.raw, str_to_chars(&mut s_path),
        str_to_chars(&mut s_own), str_to_chars(&mut s_group))}) == 0
  }
  ...
  pub fn create_with_params(
    ...
    let file = unsafe {
      let mut s_path = String::from(path);
      hdfsOpenFile(self.raw, str_to_chars(&mut s_path), O_WRONLY,
  ...
  pub fn get_hosts(&self, path: &str, start: usize, length: usize)
    ...
    let ptr = unsafe {
      let mut s_path = String::from(path);
      hdfsGetHosts(self.raw, str_to_chars(&mut s_path),
  ...
  pub fn mkdir(&self, path: &str) -> Result<bool, HdfsErr> {
    if unsafe{
      let mut s_path = String::from(path);
      hdfsCreateDirectory(self.raw, str_to_chars(&mut s_path))} == 0 {
      Ok(true)
    } else {
      Err(HdfsErr::Unknown)
    }
  ...
  pub fn open_with_bufsize(&self, path: &str, buf_size: i32)
    ...
    let file = unsafe {
      let mut s_path = String::from(path);
      hdfsOpenFile(self.raw, str_to_chars(&mut s_path), O_RDONLY,
  ...
  pub fn rename(&self, old_path: &str, new_path: &str)
    ...
    let res = unsafe {
      ...
      hdfsRename(self.raw, str_to_chars(&mut s_old_path), str_to_chars(&mut s_new_path))
    };
...
impl<'a> HdfsFsCache<'a> 
{
  ...
  pub fn get(&mut self, path: &str,port:u16) -> Result<HdfsFs<'a>, HdfsErr> 
  {
    ...
    if !map.contains_key(&namenode_uri) {  
      let hdfs_fs = unsafe {
        ...
        let mut lc = String::from("localhost");
        println!("Connecting to Namenode ({})",&namenode_uri);
        hdfsBuilderSetNameNode(hdfs_builder, str_to_chars(&mut lc));
